...
```{r}
setwd("C:\\Users\\ijohan\\Documents\\izzat_R\\Wei_Wern_Working_Files_Folders")
rm(list = ls())
```
...
```{r}
file_date_upper <- '2019-06-30'
file_date_lower <- '2019-06-01'

file_list_string <- "June2019"
```
...
complaints_file_list <- list.files("BCRM Data/Service Request and Complaints Data") %>% as.data.frame() %>% 
  filter(str_detect(., "Complaint")) %>% filter(str_detect(., file_list_string)) %>% droplevels() %>% 
...
servicereq_file_list <- list.files("BCRM Data/Service Request and Complaints Data") %>% as.data.frame() %>% 
  filter(str_detect(., "Service_Request")) %>% filter(str_detect(., file_list_string)) %>% droplevels()  %>% 
  transmute(file_names = as.character(.))
```
...
for (n in 1:nrow(complaints_file_list)){
  ...
  data_name <- paste("data_complaints_",file_list_string,"_", data_date, sep = "")
  data_name_filtered <- paste("data_complaints_",file_list_string,"_filtered_", data_date, sep = "")
  ...
  data_extract_filtered <- data_extract %>% filter(Creation_Date <= file_date_upper) %>%
    filter(Creation_Date >= file_date_lower) %>% 
...
for (n in 1:nrow(servicereq_file_list)){
  ...
  data_date <- strsplit(servicereq_file_list$file_names[n], "-")[[1]][2] %>% 
    gsub("Extract.xlsx","",.) %>% trimws(.)
  data_name <- paste("data_servicereq_",file_list_string,"_", data_date, sep = "")
  data_name_filtered <- paste("data_servicereq_",file_list_string,"_filtered_", data_date, sep = "")
...
list_df <- ls() %>% as.data.frame() %>% 
  filter(str_detect(., paste("data_complaints_",file_list_string,"_filtered",sep = ""))) %>% 
...
# The code below lists out variables in the R Global Environment, in which the names of the variables matches the below conditions, and then establishes the data frame 
list_df <- ls() %>% as.data.frame() %>% 
  filter(str_detect(., paste("data_servicereq_",file_list_string,"_filtered",sep = ""))) %>% 
  filter(!str_detect(., "bcrmInspection")) %>% 
  droplevels() %>% transmute(df = as.character(.))
...
data_sn_remark_complaint_v3 <- 'BCRM Data/SN Remark Data/Complaint_23June2019_For_NEW_SNREMARKS - 23Jun2019.xlsx'
data_sn_remark_servicereq_v3 <- 'BCRM Data/SN Remark Data/Service_Request_23June2019_For_NEW_SNREMARKS - 23un2019.xlsx'

data_sn_remark_complaint_v3 <- read_excel(data_sn_remark_complaint_v3, sheet = 1, col_names = TRUE) %>% 
  select(Complaint_ID, SN_Remark)
data_sn_remark_servicereq_v3 <- read.xlsx(data_sn_remark_servicereq_v3, sheet = 1, colNames = TRUE) %>% 
  select(Service_Request_ID, SN_Remark)

```

```{r}
data_servicereq_June2019_23Jun2019 %>% select(Service_Request_ID)
```
...
for(n in 1:nrow(list_df)){
  ...
  data_merge <- get(list_df$df[n]) %>% 
    left_join(., data_sn_remark_servicereq_v3,
...
# The code below lists out the specific data frames for which the join is to be done on
list_df <- ls() %>% as.data.frame() %>% 
  filter(str_detect(., "data_complaints_June2019_filtered")) %>%
  filter(str_detect(., "bcrmInspection")) %>% 
  filter(!str_detect(., "SNRemarks")) %>% 
  droplevels() %>% transmute(df = as.character(.))
...
for(n in 1:nrow(list_df)){
  ...
  # and then filters out and makes into NA any content with SN Remarks still containing internal notes
  data_merge <- get(list_df$df[n]) %>% 
    left_join(., data_sn_remark_complaint_v3,
              by = c("Complaint_ID")) %>% 
    mutate(notes_clean = gsub("#",'', toupper(Notes)), 
           sn_upper = toupper(`SN_Remark`)) %>% 
    rowwise() %>% 
...
```{r}
data_servicereq_June2019_filtered_23Jun2019_bcrmInspection_SNRemarks %>% filter(!is.na(clean_sn_remark))
```
...
```{r}
meter_rca_keywords <- c("QUALITY OF THE METER", "VANDALISME", "AGEING METER, QUALITY OF THE METER", "OVERLOAD / BURNT CABLE", "DAMAGED INTERNAL WIRING", "FAULTY METER", "METER HAD NO DISPLAY","BLURRY DISPLAY METER / FAULTY METER","METER GLARE","METER NO DISPLAY", "WRONG READING DUE TO BLURRY DISPLAY","FAULTY DISPLAY BUTTON","METER BROKEN")
crossing_rca_keywords <- c("CROSSED METER CONNECTION","CROSS METER INSTALLATION")
customer_rca_keywords <- c("HIGH CONSUMPTION","HIGHER CONSUMPTION","SHUTDOWN / BREAKDOWN")
...
system_data_issues_rca_keywords <- c("EMAIL BOUNCE/OMS FAIL","BILL SENT TO DIFFERENT PREMISE","INCOMPLETE CUSTOMER INFO FILLED BY CONTRACTOR","PREMIS CHANGE ADDRESS BY LOCAL AUTHORITY","WRONG STATION TAGGING","WRONG EMAIL ADDRESS","PREMISE NOT FOUND","PROBLEM ON GENARATE BILL/SMS BOUNCE")
unclassified_rca_keywords <- c("AGEING METER BOARD","LINE ABC/PVC/BARE WIRE EXPOSED/DAMAGE","VANDALISM (DAMAGE MAILBOX/NO MAILBOX)","N/A","INSTALLATION NOT COMPLY TO ESAH","ILLEGAL CONNECTION WITH ACCOUNT")
...
meter_quality_keywords <- "\\bMETER AGEINGT\\b|\\bMETER SHORTING\\b|\\bTUKAR MTR\\b|\\bMETER RALAT\\b|\\bMETER MEKANIKAL\\b|\\bNO PAPARAN\\b|\\bMETER USANG\\b|\\bMETER MEKANIKAL\\b|\\bTIDAK MENYALA\\b|\\bBEKALAN MASIH ADA\\b|\\bMETER TERHENTI\\b|\\bCAJ MINIMA\\b|\\bMETER SAMAR\\b|\\bMETER BLANK\\b|\\bMETER BROKEN\\b|\\bMETER SHORTING\\b|\\bMETER PICAH\\b|\\bGANTI METER\\b|\\bTIDAK BERGERAK\\b|\\bMETER HILANG\\b|\\bMETER PREMISE\\b|\\bSYAK\\b|\\bMETER ROSAK\\b|\\bMETER ROSK\\b|\\bMETER KOTOR\\b|\\bMETER KOTOR\\b|\\bMTR USANG\\b|\\bMETER KABUR\\b|\\bJANGKA USANG\\b|\\bOLD METER\\b|\\bCHANGE METER\\b|\\bMETER BERGERAK LAJU\\b|\\bMETER USANG\\b|\\bTIDAK BERGERAK\\b|\\bJANGKA PENGGUNA LAJU\\b|\\bTIDAK MENEPATI\\b|\\bMETER ROSAK|\\bMETER SILAU\\b|\\bDIDAPATI ROSAK\\b|\\bROSK\\b|METER USANG\\b|\\bMETER USANG|\\bAGED METER|\\bTUKAR METER BARU|ACCURACY METER OUT|\\bMETER MELEBIHI 10 TAHUN|MTR ROSAK|METER LAMA\\b|METER SLOW|\\+5|\\+4|\\-5|\\-4|\\bMETER LEBIH 10 TAHUN|BACAAN PADA METER LOMPAT|JANGKA KABUR DAN USANG|KEJANGGALAN METER AMAT TINGGI|PIRING JANGKA TAK STABIL|RALAT METER TIDAK MENGIKUT|SUDAH TUKAR JANGKA BARU|\\bERROR TINGGI|METER LAJU|METER ROSAK|METER USANG|METER BLANK/BURNT|READING STOP|ROSAK-BLANK|GERAK LAJU|JANGKA LAJU|JANGKA BARU|GANTI METER|USANG|AGED|TERHENTI|METER PECAH|\\-8|METER TIADA PAPARAN|METER BERHENTI|ERROR 32.98%|TIADA PAPARAN|RDG STOP|METER TERBAKAR|METER BLANK|METER DAN PREMIS TERBAKAR|DISPLAY ROSAK|JANGKA ROSAK|BLANK|METER PERLAHAN\\b|\\bCHOYKWHMETER ROSAK\\b|\\bSEMAK METER ROSAK\\b|\\bMETER ROSAK X\\b|\\bMETER ROSAKRUMAH TIADA\\b|\\bMETER ROSAK MINTAHUBUNGI\\b|\\bMETER ROSAK STOPCUST\\b|\\bLAB TEAMMETER ROSAKPERLU\\b|\\bMETER ROSAKSEMAKN DAPATI\\b|\\bPERUKARAN METER ROSAKTIADA\\b|\\bMETER ROSAKTERHENTI PUAN\\b|\\bMETER ROSAK TQ\\b|\\bMETER ROSAK PERLU\\b|\\bAMIRUDDINMETER ROSAK\\b|\\bMETER ROSAKBUAT\\b|\\bMETER ROSAKSEMAKN\\b|\\bMETER ROSAK PENGGUNAAN\\b|\\bBETUL RMETER ROSAKM\\b|\\bAI LEEMETER ROSAKMOHON\\b|\\bDIRUMAHTEMUJANJI ESOKMETER ROSAKTUKAR\\b|\\bSALAHATAU METER ROSAK\\b|\\bDISYAKI METER ROSAKRUMAH\\b|\\bJULIANA METER ROSAK\\b|\\bTEAMMETER ROSAKPERLU\\b|\\bKWHMETER ROSAKTRANSFER\\b|\\bFAKIRRADZILR KWHMETER ROSAKAVERAGE\\b|\\bKUATIR METER ROSAK\\b|\\bMETER ROSAK DISEBABKAN\\b|\\bMETER ROSAKPENGGUNA BERITAHU\\b|\\bENMUNIADY METER ROSAK\\b|\\bMETER ROSAK SERTA\\b|\\bAPPOINTMENT METER ROSAK\\b|\\bDISYAKI METER ROSAKADA\\b|\\bMETER ROSAKTRANSFER KE\\b|\\bRISAU METER ROSAK\\b|\\bMETER ROSAKSEMAKA TELAH\\b|\\bDIKHUATIRI METER ROSAK\\b|\\bNURUL METER ROSAK\\b|\\bLOONG METER ROSAK\\b|\\bMETER ROSAKADA\\b|\\bMETER ROSAKBUAT TUKAR\\b|\\bSUSPECT METER ROSAKNO\\b|\\bTERDAPAT METER ROSAK\\b|\\bMETERR METER ROSAKM\\b|\\bMETER ROSAKBACAAN TIDAK\\b|\\bMETER ROSAKM INTERNAL\\b|\\bMETER ROSAKRUMAH\\b|\\bMETER ROSAKDAN CUST\\b|\\bTET CHOYKWHMETER ROSAK\\b|\\bPERLAHAN METER ROSAK\\b|\\bMETER ROSAK PEGGUNA\\b|\\bAMIRUDDINMETER ROSAK LAST\\b|\\bMENDAPATI METER ROSAKGANTI\\b|\\bFAULFINDER METER ROSAK\\b|\\bBBRQ METER ROSAK\\b|\\bSIVAM RMETER ROSAKM\\b|\\bSAIDRMMETER ROSAK\\b|\\bMR VISHALMETER ROSAK\\b|\\bMETER ROSAK NUMBER\\b|\\bMIZAMETER ROSAK CEPER\\b|\\bTERHENTIBIL METER ROSAK\\b|\\bROSAKMETER ROSAKTUKAR SERTA\\b|\\bMETER ROSAK MISS\\b|\\bJANGKA METER ROSAKPURATA\\b|\\bDITAPAKMETER ROSAK ERROR\\b|\\bSYAKI METER ROSAKBIL\\b|\\bMETER ROSAK BULA\\b|\\bMETER ROSAKERROR\\b|\\bDAN METER ROSAK\\b|\\bMETER ROSAK TERKENA\\b|\\bTERPASANGDISYAKI METER ROSAKMOHON\\b|\\bMETER ROSAK ATAUSEBAGAINYAADUAN\\b|\\bTERTULIS METER ROSAK\\b|\\bMOHDMETER ROSAKTIADA\\b|\\bMETER ROSAK ASSIGN\\b|\\bMETER ROSAKLAJU INTERNAL\\b|\\bMETER ROSAK TNB\\b|\\bLCD METER ROSAK\\b|\\bSUSPEK METER ROSAKLAJU\\b|\\bLEEMETER ROSAKMOHON TEAM\\b|\\bSUSPECTS METER ROSAK\\b|\\bMAKLUM METER ROSAKTERHENTI\\b|\\bYANG DIBENARKANMETER ROSAKSUDAH\\b|\\bMETER ROSAKSAYA MULA\\b|\\bMETER ROSAKMETER JUMPTUKAR\\b|\\bKE METER ROSAK\\b|\\bMETER ROSAKKIV\\b|\\bRAHMAN METER ROSAKMETER\\b|\\bMETER ROSAK HARAP\\b|\\bVISHALMETER ROSAK\\b|\\bLAJU METER ROSAK\\b|\\bBAIKMETER ROSAK\\b|\\bFC METER ROSAK\\b|\\bPENGGUNA METER ROSAKTIADA\\b|\\bINFO METER ROSAK\\b|
\\bARTHYMETER ROSAKTARIKHMASA PMSEMAKKAN"

no_reading_billing_keywords <- "\\bBILL XDAPAT\\b|\\bLAMA TAK TERIMA\\b|\\bTIDAK DAPAT BIL\\b|\\bTIADA BACAAN\\b|\\bBIL TAK TERIMA|\\bX DPT BI LL"

meter_crossing_keywords <- "\\bSALAH PASANG\\b|\\bTERSALAH DIPASANG\\b|\\bJANGKA HILANG\\b|\\bmeter crossing\\b|\\bSILANG\\b|\\bCROSSING\\b|\\bCROSING\\b|\\bBERSILANG\\b"
...
incorrect_reading_keywords <- "\\bADALAH TIDAK TEPAT\\b|\\bBACAN SALAh\\b|\\bADALAH TIDAKTEPAT\\b|\\bWRONG KEY IN\\b|\\bTERSALAH AMBIL BACAAN\\b|\\bBACA AN SALAH\\b|\\bBACA SALAH\\b|\\bPEMBETULAN BACAAN\\b|\\bPEMBETULAN BA CAAN\\b|\\bBACAAN SALAH OLEH MR\\b|\\bRALAT BACAAN\\b|\\bBUAT BILL BARU\\b|\\bBACAAN SALA\\b|\\bKESILAPAN BACAAN\\b|\\bSALAH BACAAN\\b|BACAAN SALAH\\b|\\bBETULKAN BACAAN\\b|\\bBIL BACA SALAH\\b|\\bBACAAN SA LAH\\b|\\bSALAH BACA\\b|\\bBACAAN ADALAH SALAH\\b|\\bSALAH RDG\\b|\\bMETEREDER SALAH\\b|\\bKESILAPAN\\b|\\bOVEREADING|\\bSALAH MEMBUAT BACAAN\\b|\\OVERREAD|RDG TINGGI\\b|\\bO/READ|BACAAN SUDAH DIBETULKAN\\b|\\bTERSALAH MENGAMBIL BACAAN|\\bDIBETULKAN DENGAN BACAAN SEBENAR|BACCAN SUDAH DIBETULKAN|SALAH MMBUAT BACAAN|WRONG READING|BACAAN KVARH SALAH|BILL TELAH DIBETULKAN MENGIKUT BACAAN|OVER READ|\\bADALAH SALAH|SALAH READING\\b|RDG OVERRE AD|\\bPEMBACA JANGKA TERSALAH|BACAAN METER SALAH|MR TERKURANG BACA\\b|BACAAN MR BULAN 3 RENDAH|OVEREAD READING|TERSALAH BUAT BACAAN|SALAH BACAQN DITAPAK|BACAAN TIDAK JELAS|\\bKESALAHAN BACAAN|BACAAN TINGGI OLEH P\\/JANGKA|BCN BLN 04 SALAH|BACAAN UNDER READ|BACAAN MAC DAN APRIL MINIMUM CAJ|SALAH TECO READING|;BACAAN BULAN LEPAS RENDAH|BIL TERSILAP BACA|BACAAN ADALAH KOSONG|BACAAN APRIL RENDAH|BACAAN DIBACA RENDAH|SALINAN BILL SETELAH MEMBUAT SEMAKAN SEMULA|TERSALAH BACA|TEROPONG|SALAH BACCAN|SILAP BACA|TERSALAH MASUKKAN BACAAN|BACAAN DAHULU SALAH|BACAAN TINGGI SALAH|PEMBACA JANGKA BACA SALAH|BACAAN TINGGI OLEH P\\b|\\bTINGGIDAN TIDAK TEPAT\\b|\\bYG TIDAK TEPAT\\b|\\bTIDAK TEPATPENGUNA\\b|\\bADALAH TIDAK TEPATPENGUNA\\b|\\bTIDAKTEPATPENGGUNA MEM\\b|\\bTIDAK TEPATMOHON\\b|\\bPENGGUNA TIDAK TEPAT\\b|\\bADALAH TIDAKTEPATPENGGUNA\\b|\\bTIDAK TEPAT PENJELASAN\\b|\\bTIDAK TEPATPENGGUNA\\b|\\bTIDAK TEPATPEMB ETULAN\\b|\\bTEPAT TERATUR TIDAK\\b|\\bDIDAPATI BACAAN BETULTIDAK\\b|\\bBACAAN TIDAK BOLEHDIBETULKAN\\b|\\bBACAAN BETUL TIDAK\\b|\\bBACAAN PEMBETULAN TIDAK\\b|\\bBACAAN BETULTIDAK\\b|\\bTIDAK BERAPA BETULPERGERAKAN\\b|\\bDISEMAKBACAAN BETULKWHPENGGUNA TIDAK\\b|\\bBACAAN BETULTERATURPENGGUNA TIDAK\\b|\\bBACAAN BETULTIDAK DAPAT\\b|\\bTINGGIDAN TIDAK TEPAT\\b|\\bYG TIDAK TEPAT\\b|\\bTIDAK TEPATPENGUNA\\b|\\bADALAH TIDAK TEPATPENGUNA\\b|\\bTIDAKTEPATPENGGUNA MEM\\b|\\bTIDAK TEPATMOHON\\b|\\bPENGGUNA TIDAK TEPAT\\b|\\bADALAH TIDAKTEPATPENGGUNA\\b|\\bTIDAK TEPAT PENJELASAN\\b|\\bTIDAK TEPATPENGGUNA\\b|\\bTIDAK TEPATPEMB ETULAN\\b|\\bTEPAT TERATUR TIDAK\\b|\\bDIDAPATI BACAAN BETULTIDAK\\b|\\bBACAAN TIDAK BOLEHDIBETULKAN\\b|\\bBACAAN BETUL TIDAK\\b|\\bBACAAN PEMBETULAN TIDAK\\b|\\bBACAAN BETULTIDAK\\b|\\bTIDAK BERAPA BETULPERGERAKAN\\b|\\bDISEMAKBACAAN BETULKWHPENGGUNA TIDAK\\b|\\bBACAAN BETULTERATURPENGGUNA TIDAK\\b|\\bBACAAN BETULTIDAK DAPAT"

#incorrect_reading_keywords <- "\\bBACAAN SALA\\b|\\bKESILAPAN BACAAN\\b|\\bSALAH BACAAN\\b|BACAAN SALAH\\b|\\bBETULKAN BACAAN\\b|\\bBIL BACA SALAH\\b|\\bBACAAN SA LAH\\b|\\bSALAH BACA\\b|\\bBACAAN ADALAH SALAH\\b|\\bSALAH RDG\\b|\\bMETEREDER SALAH\\b|\\bKESILAPAN\\b|\\bOVEREADING|\\bSALAH MEMBUAT BACAAN\\b|\\OVERREAD|RDG TINGGI\\b|\\bO/READ|BACAAN SUDAH DIBETULKAN\\b|\\bTERSALAH MENGAMBIL BACAAN|\\bDIBETULKAN DENGAN BACAAN SEBENAR|BACCAN SUDAH DIBETULKAN|SALAH MMBUAT BACAAN|WRONG READING|BACAAN KVARH SALAH|BILL TELAH DIBETULKAN MENGIKUT BACAAN|OVER READ|\\bADALAH SALAH|SALAH READING\\b|RDG OVERRE AD|\\bPEMBACA JANGKA TERSALAH|BACAAN METER SALAH|MR TERKURANG BACA\\b|BACAAN MR BULAN 3 RENDAH|OVEREAD READING|TERSALAH BUAT BACAAN|SALAH BACAQN DITAPAK|BACAAN TIDAK JELAS|\\bKESALAHAN BACAAN|BACAAN TINGGI OLEH P\\/JANGKA|BCN BLN 04 SALAH|BACAAN UNDER READ|BACAAN MAC DAN APRIL MINIMUM CAJ|SALAH TECO READING|;BACAAN BULAN LEPAS RENDAH|BIL TERSILAP BACA|BACAAN ADALAH KOSONG|BACAAN APRIL RENDAH|BACAAN DIBACA RENDAH|SALINAN BILL SETELAH MEMBUAT SEMAKAN SEMULA|TERSALAH BACA|TEROPONG|SALAH BACCAN|SILAP BACA|TERSALAH MASUKKAN BACAAN|BACAAN DAHULU SALAH|BACAAN TINGGI SALAH|PEMBACA JANGKA BACA SALAH|BACAAN TINGGI OLEH P"
#\\bBILL XDAPAT\\b|\\bLAMA TAK TERIMA\\b|\\bTIDAK DAPAT BIL\\b|

missing_meter_keywords  <- "METER HILANG"

high_bill_keywords <- "\\b2 KALI GANDA"

long_billing_period_keywords <- "\\bHARI\\b|\\bMELEBINI 30 HARI\\b|\\bBCRM DOWN\\b|\\bDEBAB BACA LAMBAT\\b|\\bPRORATE\\b|\\bISU PRORATE\\b|\\bTINGGI SEBAB LAMBAT\\b|\\bLAMBAT BACA\\b|\\b32 HARI|\\b33 HARI|\\b34 HARI|\\b35 HARI|\\b36 HARI|LEBIH 30 HARI\\b|\\b62 HARI|\\bPERGIRAAN PRORATE|\\bBIL DIBACA LAMBAT|\\bPRORATA KIRAAN|MINTA BILLING 31 HARI|\\bPRORATA|35 HARI|MINTA BIL 31 HARI|BIL 34HARI|PRORATE 30 HARI|LEWAT BACAAN 33\\b|\\bPRORATA BAGI\\b|\\bBERKENAAN KIRAAN PRORATA\\b|\\bKIRAAN PRORATA BAGI\\b|\\bPRORATA BAGI BIL\\b|\\bPERBINCANGAN PRORATA\\b|\\bPRORATA BILL TINGGI\\b|\\bPERBINCANGAN PRORATA BILL\\b|\\bPRORATA BILL\\b|\\bFAKTOR PRORATA\\b|\\bBIL PRORATA\\b|\\bPENERANGAN PRORATA\\b|\\bPRORATA BIL\\b|\\bSECARA PRORATA\\b|\\bPRORATA TELAH\\b|\\bBILL PRORATA\\b|\\bPRORATA DIGUNAPAKAI\\b|\\bPRORATA DGN\\b|\\bPERBINCANGAN BILL PRORATA\\b|\\bPRORATA DGN UNIT\\b|\\bNOTE PENERANGAN PRORATA\\b|\\bBILL PRORATA DGN\\b|\\bPENGIRAAN FAKTOR PRORATA\\b|\\bFAKTOR PRORATA DIGUNAPAKAI\\b|\\bBACAAN PRORATE\\b|\\bPRORATE BERDASARKAN\\b|\\bPRORATE BERDASARKAN BACAAN\\b|\\bBACAAN PRORATE BERDASARKAN\\b|\\bMENGGUNAKAN BACAAN PRORATE\\b|\\bPRORATE KWH\\b|\\bPRORATE HARI\\b|\\bPRORATE HARI TELAH\\b|\\bSELEPAS PRORATE KWH\\b|\\bSELEPAS PRORATE\\b|\\bPRORATE BILSEKIRANYA\\b|\\bPRORATE BILSEKIRANYA PENGGUNA\\b|\\bPROSES PRORATE\\b|\\bPRORATA BAGI\\b|\\bBERKENAAN KIRAAN PRORATA\\b|\\bKIRAAN PRORATA BAGI\\b|\\bPRORATA BAGI BIL\\b|\\bPERBINCANGAN PRORATA\\b|\\bPRORATA BILL TINGGI\\b|\\bPERBINCANGAN PRORATA BILL\\b|\\bPRORATA BILL\\b|\\bFAKTOR PRORATA\\b|\\bBIL PRORATA\\b|\\bPENERANGAN PRORATA\\b|\\bPRORATA BIL\\b|\\bSECARA PRORATA\\b|\\bPRORATA TELAH\\b|\\bBILL PRORATA\\b|\\bPRORATA DIGUNAPAKAI\\b|\\bPRORATA DGN\\b|\\bPERBINCANGAN BILL PRORATA\\b|\\bPRORATA DGN UNIT\\b|\\bNOTE PENERANGAN PRORATA\\b|\\bBILL PRORATA DGN\\b|\\bPENGIRAAN FAKTOR PRORATA\\b|\\bFAKTOR PRORATA DIGUNAPAKAI\\b|\\bBACAAN PRORATE\\b|\\bPRORATE BERDASARKAN\\b|\\bPRORATE BERDASARKAN BACAAN\\b|\\bBACAAN PRORATE BERDASARKAN\\b|\\bMENGGUNAKAN BACAAN PRORATE\\b|\\bPRORATE KWH\\b|\\bPRORATE HARI\\b|\\bPRORATE HARI TELAH\\b|\\bSELEPAS PRORATE KWH\\b|\\bSELEPAS PRORATE\\b|\\bPRORATE BILSEKIRANYA\\b|\\bPRORATE BILSEKIRANYA PENGGUNA\\b|\\bPROSES PRORATE"
...
smart_meter_keywords <- "\\bSMARTMETER\\b|\\bSMART\\b|\\bSMART METER\\b|\\bSMART METER|SMART METER\\b|\\bSMART|BILIK GERAKAN SM|UNIT SM"
...
estimate_bill_keywords <- "\\bTIDAK DAPAT MEMBUAT BACAAN\\b|\\bOVER READ\\b|\\bUNDER READ\\b|\\bDBILL N\\b|\\bBCAAN ANGARAN\\b|\\bBIL ANG GARAN\\b|\\bBACAAN EST\\b|\\bBACAAN AGGARAN\\b|\\bBIL N\\b|\\bBACAAN SEBENAR\\b|\\bBACAAN ANGARAN\\b|\\bBECAAN SEBENAR TELAH\\b|\\bBECAAN SEBENAR SUDAH\\b|\\bMOHON BUAT BILL N\\b|\\bE BILL\\b|\\bBIL E\\b|\\bPEMBILAN SEBENAR\\b|\\bTERIMA BIL\\b|\\bPEMBILAN SEMULA\\b|\\bBACAAN ANGGARAN\\b|\\bBACAAN SEBENAR\\b|\\bBIL ANGGARAN\\b|\\bANGGARAN\\b|BACAAN ESTIMATE\\b|\\bSYSTEM EST RENDAH|BACAAN RENDAH PADA BULAN AP RIL\\b| ESTIMATE|TERIMA BIL ELEKTRIK ESTI MATE|TERIMA BIL ELEKTRIK ESTIMA TE|ANG GARAN OLEH SM|BIL BARU|BILL BARU|TERIMA BIL ANGGA RAN\\b|\\bBILANGGARAN PADA TARIKH\\b|\\bBACAAN ANGGARANNO TEL\\b|\\bMR WILLIAMBIL ANGGARAN\\b|\\bTINGGI ANGGARAN BIL\\b|\\bBACAAN ANGGARANBACAAN TERKINI\\b|\\bBACAANSEBENARBIL ANGGARAN LAWATAN\\b|\\bAMBIL BACAAN ANGGARAN\\b|\\bANGGARANTELAH AMBIL\\b|\\bBIL ANGGARANPEM BETULAN\\b|\\bKWHBIL ANGGARAN BIL\\b|\\bDIAMBIL MENGGUNAKAN BACAANANGGARAN\\b|\\bMOHD RAZALIBACAAN ANGGARAN\\b|\\bPERGI BIL ANGGARANTELAH\\b|\\bNYA ANGGARAN RM\\b|\\bANGGARAN TTP PENGGUNA\\b|\\bSECARA ANGGARAN PENGGUNA\\b|\\bANGGARAN BILL\\b|\\bANGGARAN BERJUMLAH\\b|\\bBIL ANGGARANTELAH\\b|\\bANGGARANDAN BUA T\\b|\\bPREMIS BACAAN ANGGARANTARIKHMASAPMBIL\\b|\\bANGGARAN TUDOUBLE\\b|\\bKIRAAN PENGGUNAAN ANGGARAN\\b|\\bBIL ANGGARANSILA\\b|\\bANGGARAN TELAH PERGI\\b|\\bBIL ANGGARANMEMBATALKAN\\b|\\bKERANAMENDAPAT BIL ANGGARAN\\b|\\bANGGARAN DAN PADA\\b|\\bTU ANGGARAN TUDOUBLE\\b|\\bSALLEH BIL ANGGARANPERLU\\b|\\bBIL ANGGARANRUMAH\\b|\\bBUAT ANGGARAN UNTUK\\b|\\bANGGARAN SETELAH BIL\\b|\\bDARI BILANGGARAN SEDANGKAN\\b|\\bBIL ANGGARANRUMAH KEDAI\\b|\\bANGGARAN SERTA BIL\\b|\\bANGGARAN KAMARUL AZH\\b|\\bYANG ANGGARAN\\b|\\bANGGARANBACAAN TELAH DI\\b|\\bBAGI APRIL BACAANANGGARAN\\b|\\bANGGARANTIADA REKOD\\b|\\bBACAAN ANGGARAN TETAPI\\b|\\bANGGARANBUAT TEMUJANJI PADA\\b|\\bANGGARAN MOHON SEMAK\\b|\\bANGGARAN SAHAJA APRIL\\b|\\bTERDAPAT BIL ANGGARAN\\b|\\bTULIS ANGGARAN DAN\\b|\\bANGGARANBACAAN SEBENAR PADA\\b|\\bBIL ANGGARANPENGGUNA MOHON\\b|\\bSEBENARBACAAN ANGGARAN OLEH\\b|\\bKERANA BIL BACAANANGGARANMOHON\\b|\\bCUST RDGBIL ANGGARAN\\b|\\bDIBUATSECARA ANGGARAN DAN\\b|\\bKHAMIS ANGGARAN DLM\\b|\\bANGGARANCUST BAWA\\b|\\bBILL ANGGARAN PADA\\b|\\bBIL ANGGARAN YAG\\b|\\bBIL ANGGARAN DIKELUARKANCORAK\\b|\\bTEPAT DAN BETULANGGARAN\\b|\\bANGGARANBACAAN TELAH\\b|\\bDAN BERI ANGGARAN\\b|\\bBIL ANGGARAN SEMENJAK\\b|\\bDIKIRA ANGGARAN\\b|\\bANGGARANTEL NAMA\\b|\\bADALAHANGGARAN PENERANGAN\\b|\\bSAHAJABACAAN ANGGARAN\\b|\\bSEDANGKAN PENGGUNABULANAN ANGGARAN\\b|\\bKIRAAN BACAAN ANGGARAN\\b|\\bMAKLUM ANGGARAN PENGGUNAAN\\b|\\bANGGARAN AKANTETAPI PENGGUNA\\b|\\bADALAH BIL ANGGARANPENGIRAAN\\b|\\bDAPAT BIL ANGGARAN\\b|\\bTAPAK BACAAN ANGGARAN\\b|\\bSEBAB BIL ANGGARAN\\b|\\bANGGARAN KERANA UNTUK\\b|\\bBACAANANGGARAN OLEH SISTEM\\b|\\bBIL ANGGARANBIL HB\\b|\\bNOTE BIL ANGGARANBIL\\b|\\bANGGARAN TEST METER\\b|\\bBIL ANGGARANBACAAN NORMAL\\b|\\bANGGARAN TERLALU TINGGIMOHON\\b|\\bADUAN PENGGUNABIL ANGGARAN\\b|\\bBULAN BIL ANGGARANSMART\\b|\\bBACAAN ANGGARANTARIKHMASAPMBIL ANGGARAN\\b|\\bANGGARANPENGIRAAN FAKTOR PRORATA\\b|\\bKUMAREFFENDYBACAAN ANGGARAN AKI\\b|\\bANGGARAN KE INTERNAL\\b|\\bDLAM ANGGARAN\\b|\\bSEBENARBIL ANGGARAN TINGGINO\\b|\\bANGGARANBIL BULAN MAY\\b|\\bBIL TERSEBUTADALAH ANGGARAN\\b|\\bPENGGUNABULANAN ANGGARAN RM\\b|\\bBULAN APRILBACAAN ANGGARANBIL\\b|\\bBIL ANGGARANMOHON BUAT\\b|\\bBELIAU DALAM ANGGARAN\\b|\\bBILLINGISU BIL ANGGARAN\\b|\\bTERIMA KASIHLINABIL ANGGARAN\\b|\\bNOTE BACAAN ANGGARANREADING\\b|\\bPENGGUNABIL ANGGARAN\\b|\\bPIHAK TUANPUANMOHAMADBILL ANGGARANLAWATAN\\b|\\bWILLIAMBIL ANGGARAN ANGGARAN\\b|\\bINI ANGGARAN BILRUMAH\\b|\\bBIL ANGGARANTETAPI PENGGUNA\\b|\\bAZAMBIL ANGGARAN TELAH\\b|\\bADALAH BACAAN ANGGARANTELAH\\b|\\bLAGI ANGGARAN INTERNAL\\b|\\bBACAAN ANGGARANPENGGUNA TAK\\b|\\bBACAANANGGARAN DAN\\b|\\bIMANBIL ANGGARAN TINGGI\\b|\\bBIL ANGGARAN SEHINGGA\\b|\\bAPRILBACAAN ANGGARANBIL\\b|\\bANGGARANRM RM DI\\b|\\bANGGARAN BILTOLONG SEMAK\\b|\\bPEMBATALAN BILBIL ANGGARANDAN\\b|"
...
```{r}
complaints_rca_mart_23Jun2019_wflag <-
data_complaints_June2019_filtered_23Jun2019_bcrmInspection_SNRemarks %>%
...
```{r}
complaints_rca_mart_23Jun2019_wflag %>% filter(sum_rca_flag == 0) %>% 
  filter(Root_Cause_Analysis != "NOT ASSIGNED") %>% 
  group_by(Root_Cause_Analysis) %>% 
...
```{r}
complaints_snremark_mart_23Jun2019_wflag_rca <-
complaints_rca_mart_23Jun2019_wflag %>% 
...
```{r}
servicereq_SNremark_mart_23Jun2019_wflag <-
data_servicereq_June2019_filtered_23Jun2019_bcrmInspection_SNRemarks %>% 
  mutate(clean_sn_remark = trimws(clean_sn_remark),
...
write.csv(servicereq_SNremark_mart_23Jun2019_wflag, file = "UPDATED_WITH24JUNSTATUS_INFO_SERVICEREQ_SNREMARK_ANALYSIS_24JUNE2019V1.csv")

#write.csv(complaints_snremark_mart_23Jun2019_wflag_rca, file = #"UPDATED_WITH23JUNSTATUS_INFO_COMPLAINTS_RCA_SNREMARK_ANALYSIS_24JUNE2019.csv")

```{r}
servicereq_SNremark_mart_23Jun2019_wflag %>% 
  write.xlsx(., "UPDATED_WITH24JUNSTATUS_INFO_SERVICEREQ_SNREMARK_ANALYSIS_24JUNE2019_izzat.xlsx")
```

```{r}
complaints_snremark_mart_23Jun2019_wflag_rca %>% 
  write.xlsx(., "UPDATED_WITH24JUNSTATUS_INFO_COMPLAINTS_RCA_SNREMARK_ANALYSIS_24JUNE2019_izzat.xlsx")
```
