...
#package(package_name) to install new package
```{r, message=FALSE}
require(tidyverse)
require(readxl)
require(stringr)
...
#list.files() to all the files in your working directory.
...
#creating a vector
#check number of elements length() function
#variable_name[3] accessing elements in vector
#variable_name[c(2,3,4)] multiple elements
```{r}
...
#https://statistical-programming.com/nrow-r-function/
#The nrow R function returns the number of rows that are present in a data frame
#https://hbs-rcs.github.io/R_Intro-gapminder/data-io/
paste()
```{r, message = FALSE}
#will give the number of rows inside df (n in 1:nrow(complaints_file_list))
for (n in 1:nrow(complaints_file_list)){
  
  # This establishes the full path of   the file
  full_file_path <- paste("BCRM Data/Service Request and Complaints Data/",
                          complaints_file_list$file_names[n], sep = "" )
...
list_df <- ls() %>% as.data.frame() %>% 
  filter(str_detect(., "data_complaints_Jun2019_filtered")) %>% 
...
# The code below lists out variables in the R Global Environment, in which the names of the variables matches the below conditions, and then establishes the data frame 
list_df <- ls() %>% as.data.frame() %>% 
  filter(str_detect(., "data_servicereq_Jun2019_filtered")) %>% 
  filter(!str_detect(., "bcrmInspection")) %>% 
  droplevels() %>% transmute(df = as.character(.))
...
data_sn_remark_may2019_complaint_v3 <- 'BCRM Data/SN Remark Data/Complaint_23June2019_For_NEW_SNREMARKS - 23Jun2019.xlsx'
data_sn_remark_may2019_servicereq_v3 <- 'BCRM Data/SN Remark Data/Service_Request_23June2019_For_NEW_SNREMARKS - 23un2019.xlsx'
...
no_anomaly_meter_okay_keywords <- "\\bOKRALAT\\b|AIKRALAT\\b|\\bKEADAAN BAIK|\\bJANGKA BAIK\\b|BAIK\\b|METER OK\\b|GOOD CONDITION\\b|\\bNORMALRALAT\\b|\\bTIDAK ROSAK\\b|\\bJANGKA OK\\b|\\bMTR OK|MTR OK\\b|JANGKA BAIK|DISAHKAN TIADA MASAALAH\\b|\\+0|\\-0|0\\.|\\-1|\\+1|0\\%|\\bDALAM RALAT YANG DIBENARKAN|\\bTIADA KEJANGGALAN METER|RALAT % ERROR TIDAK MELEBIHI JULAT|BAIKERROR+2.03%|KEJITUAN\\: 1"
...
data_complaints_2019_filtered_2Jun2019_bcrmInspection_SNRemarks %>%
